...
////////////////////////////////////////////////////////////////////////////////
    ...
    inline uint warpScanInclusive(uint idata, __local uint *l_Data, uint size){
		...
		if (size >= 2) {
			uint res = l_Data[pos] + l_Data[pos - 1];
			l_Data[pos] = res;
		}
		//l_Data[pos] += l_Data[pos -  1];
        if(size >=  4) {
			uint res = l_Data[pos] + l_Data[pos - 2];
			l_Data[pos] = res;
		} 
		//l_Data[pos] += l_Data[pos -  2];
        if(size >=  8) {
			uint res = l_Data[pos] + l_Data[pos - 4];
			l_Data[pos] = res;
		}
			//l_Data[pos] += l_Data[pos -  4];
        if(size >= 16) {
			uint res = l_Data[pos] + l_Data[pos - 8];
			l_Data[pos] = res;
		}
			//l_Data[pos] += l_Data[pos -  8];
        if(size >= 32) {
			uint res = l_Data[pos] + l_Data[pos - 16];
			l_Data[pos] = res;
		}
		//l_Data[pos] += l_Data[pos - 16];
...
void scanExclusiveLocal2(
	...
	uint odata = 0;
	if (get_global_id(0) < N)
		odata = scan1Exclusive(data, l_Data, arrayLength);
