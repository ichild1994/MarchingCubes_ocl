...
initMC(int argc, char** argv)
{
	...
	cameraDistance = 8.0 * mc_halfBound[2];
	...
	float* h_volumeF = loadRawFilef(path, size * sizeof(float));
	oclCheckErrorEX(h_volumeF != NULL, true, pCleanup);
...
void motion(int x, int y)
{
    ...
    if (mouse_buttons==1) {
		...
		if (mc_translate[0] > 1.0)   mc_translate[0] = 1.0;
		if (mc_translate[0] < -1.0)  mc_translate[0] = -1.0;
		if (mc_translate[1] > 1.0)   mc_translate[1] = 1.0;
		if (mc_translate[1] < -1.0)  mc_translate[1] = -1.0;
		...
		if (mc_mode == ORTHO) {
			...
			if (mc_translate[2] < -3.0*mc_halfBound[2])  mc_translate[2] = -3.0*mc_halfBound[2];
		}
    }
...
void reshape(int w, int h)
{
	...
	if (mc_mode == ORTHO)
	{
		if (w > h)
			...
			glOrtho(-2.0*float(w) / float(h), 2.0*float(w) / float(h), -2.0, 2.0, -(-mc_halfBound[2]), -(-cameraDistance - 2.0*mc_halfBound[2]));
		else
			//glOrtho(-2.0, 2.0, -2.0*float(h) / float(w), 2.0*float(h) / float(w), -(-cameraDistance + 1000), -(-cameraDistance - 1000));
			glOrtho(-2.0, 2.0, -2.0*float(h) / float(w), 2.0*float(h) / float(w), -(-mc_halfBound[2]), -(-cameraDistance - 2.0*mc_halfBound[2]));
	}
	else if (mc_mode == PERSPECTIVE)
	{
		if (w > h)
			gluPerspective(60.0f, float(w) / float(h), -(-mc_halfBound[2]), -(-cameraDistance - 2.0*mc_halfBound[2]));
		else
			gluPerspective(2.0*atan(float(h) / float(w)*0.57735)*180.0 / mc_PI, float(w) / float(h), -(-mc_halfBound[2]), -(-cameraDistance - 2.0*mc_halfBound[2]));
	}
	glMatrixMode(GL_MODELVIEW);
