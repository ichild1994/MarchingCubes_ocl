...
const float DEG2RAD = 3.1415926535898f / 180;
...
void Matrix4::setPerspectiveY(float fovy, float aspect, float zNear, float zFar) {
	assert(abs(aspect - std::numeric_limits<float>::epsilon()) > static_cast<float>(0));
	this->zero();
	float const tanHalfFovy = tan(DEG2RAD * fovy / static_cast<float>(2));
...
void Matrix4::setPerspectiveX(float fovx, float aspect, float zNear, float zFar) {
	assert(abs(aspect - std::numeric_limits<float>::epsilon()) > static_cast<float>(0));
	this->zero();
